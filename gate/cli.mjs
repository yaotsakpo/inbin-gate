...
if (cmd === "intent" && rest[0] === "--clear") { rmSync(join(HOME, "intent.json"), { force: true }); console.log("intent cleared"); }
  ...
  const j = JSON.parse(execSync(`gh ${isPr ? "pr" : "issue"} view ${num} --repo ${repo} --json number,title,body,author,authorAssociation,url`, { encoding: "utf8" }));
  const rec = { url: j.url, number: j.number, title: j.title, body: j.body, author: j.author?.login, authorAssociation: j.authorAssociation };
