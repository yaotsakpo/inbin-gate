...
run() { # id, prompt
  local id=$1; shift
  echo "=== $id ==="
  bob run --accept-license --trust -w "$PWD" --format json --max-turns 40 ${MAX_COST:+--max-cost "$MAX_COST"} $TEAM "$@" \
    | tee "bob_sessions/shell-logs/${id}.json" | tail -c 1200; echo
}
...
run B4 "Review the uncommitted changes in this repository as a careful code reviewer: correctness, security of the gate (can any change let an unestablished value through?), and test coverage. Fix anything you find, run 'npm test' and 'npm run bench', and write a short review summary to docs/review-B4.md."
run B5 "Stage all changes and write a conventional commit message describing them accurately. Commit with git. Do not push."
